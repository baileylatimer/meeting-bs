...
export function StatsTicker({ decisions, actionItems, deferredItems }: StatsTickerProps) {
  ...
  return (
    <div className="flex border-x-2 border-b-2 border-ink" style={{ backgroundColor: "#EDE8E1" }}>
      {stats.map((stat, i) => (
        <div
          key={stat.label}
          className="flex items-center justify-between flex-1 px-3 py-1.5"
          style={{
            borderRight: i < stats.length - 1 ? "1px solid #D5D0CB" : "none",
          }}
        >
